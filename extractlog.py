#!/usr/bin/env python3
"""
Extraction des informations présentes dans les logs récupérés
sur le drone pour en faire un fichier exploitable par Potree
"""
...
def valid_latitude(angle):
    """ Vérifie si la latitude est bien comprise entre -180° et +180°
    """
    value = float(angle)
...
def valid_longitude(angle):
    """ Vérifie si une longitude est bien comprise entre -90° et +90°
    """
    value = float(angle)
...
parser.add_argument(
    "-i", "--infile",
    help="Nom du fichier de log (AVEC son extension) 'GPSLOG.TXT' par défaut",
    default="GPSLOG.TXT")
parser.add_argument(
    "-o", "--outfile",
    help="Nom des fichiers de sortie (SANS extension) 'test' par défaut",
    default="test")
parser.add_argument(
    "-linear",
    action="store_true",
...
for line in lines:
    ...
    if line.startswith("$GPGGA"):
        ...
        fields = line.split(",")
        # On vérifie que ue TOUS les 16 champs sont présents
        # et que le GPS est correct
        ...
        if (len(fields) == 16) and (fields[6] != '0'):
            ...
            # au format texte
            extractfileXYZ.write(
                " ".join([str(east),
                          str(north),
                          str(up),
                          sensorData]) + "\n")
            # Détermination de la couleur discrète correspondant
            # à la valeur de la donnée
            color = getColor(float(sensorData))
            extractfileRGB.write(
                " ".join([str(east),
                          str(north),
                          str(up),
                          color]) + "\n")
